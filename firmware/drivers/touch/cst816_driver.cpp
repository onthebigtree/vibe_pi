...
CST816Driver::CST816Driver(uint8_t sda, uint8_t scl, uint8_t intr, uint8_t rst,
                           uint16_t screen_w, uint16_t screen_h)
    : _sda(sda), _scl(scl), _int(intr), _rst(rst), _w(screen_w), _h(screen_h) {}

bool CST816Driver::init() {
    ...
    Serial.printf("[CST816] Init %s\n", ok ? "OK" : "FAIL");
    return ok;
}

TouchPoint CST816Driver::read() {
